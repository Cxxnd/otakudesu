...
import type { AvailableClient, ClientId, TargetId } from '../snippetz/index.js';
import type { PluginAuthState } from './api-reference-plugin.js';
...
export declare const DEFAULT_MODELS_SECTION_LABEL: ModelsSectionLabel;
...
type TranslationDotPaths<T, Prefix extends string = ''> = {
    [Key in keyof T & string]: T[Key] extends string ? `${Prefix}${Key}` : T[Key] extends Record<string, unknown> ? TranslationDotPaths<T[Key], `${Prefix}${Key}.`> : never;
}[keyof T & string];
...
type ExtendedConfiguration = {
     ...
     */
    hiddenClients?: Partial<Record<TargetId, boolean | ClientId<TargetId>[]>> | Array<TargetId | ClientId<TargetId> | AvailableClient> | true;
